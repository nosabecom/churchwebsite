...
readonly hook_key="WOMAN_EXCEL_VERCEL_DEPLOY_HOOK_URL"
...
show_help() {
    cat <<'EOF'
Configure the Woman Excel Vercel deploy hook on the Sanity development Function.

Usage:
  pnpm blueprint:configure:womanexcel [--yes]

The script prompts for the hook without echoing it. Alternatively, export
WOMAN_EXCEL_VERCEL_DEPLOY_HOOK_URL before running it. The secret is sent directly
...
hook_url="${WOMAN_EXCEL_VERCEL_DEPLOY_HOOK_URL:-}"
if [[ -z "$hook_url" ]]; then
    if [[ ! -t 0 ]]; then
        printf '%s is unset and an interactive terminal is required.\n' "$hook_key" >&2
        exit 1
    fi
    read -r -s -p "Woman Excel Vercel deploy hook URL: " hook_url
    printf '\n'
fi
