...
const configuredPaths = Object.keys(config.assets).sort()
const unclassified = scannedPaths.filter((file) => !config.assets[file])
...
for (const [sourcePath, metadata] of Object.entries(config.assets)) {
  ...
  if (metadata.possibleDuplicateOf && !config.assets[metadata.possibleDuplicateOf]) {
...
for (const absolutePath of scannedFiles) {
  ...
  const metadata = config.assets[sourcePath]
